...
Main_init:
	...
	jr	$ra	
				...
				#Generating code for method fact in class Main
Main.fact:
	addiu	$sp $sp -8
	sw	$fp 0($sp)
	addiu	$sp $sp -4
	...
	addiu	$fp $sp 16
	move	$s0 $a0
				#Generating inner code for method fact with AR_size of 16
...
label0:
	...
	sw	$a0 0($sp)
	addiu	$sp $sp -4
				#BEGIN dispatch for method fact in class Main
...
label4:
	...
	lw	$t1 28($t1)
	jalr	$t1
				#DONE dispatch for method fact in class Main
	jal	Object.copy
	lw	$t1 4($sp)
	lw	$t1 12($t1)
	lw	$t2 12($a0)
...
label2:
				...
				#Done Generating inner code for method fact with AR_size of 16
