...
Main.main:
	...
	move	$s0 $a0
				...
				#BEGIN dispatch for method type_name in class Bool
				#Entered cgen for not
				#Entered cgen for new
	la	$a0 Bool_protObj
	jal	Object.copy
	jal	Bool_init
...
label3:
	...
	lw	$t1 4($t1)
	jalr	$t1
				#DONE dispatch for method type_name in class Bool
	sw	$a0 0($sp)
	addiu	$sp $sp -4
...
label7:
	...
	jalr	$t1
				...
				#BEGIN dispatch for method out_string in class IO
				#Evaluating and pushing argument of type String to current frame
				#BEGIN dispatch for method type_name in class String
...
label9:
	...
	lw	$t1 4($t1)
	jalr	$t1
				#DONE dispatch for method type_name in class String
	sw	$a0 0($sp)
	addiu	$sp $sp -4
				#Done pushing argument of type String to current frame
				#Entered cgen for object: io
