...
A_init:
	...
	move	$s0 $a0
				#Leaving objectInitPrologue
	jal	Object_init
				#Generating code for attribute x of type SELF_TYPE in class A
				#Entered and exited cgen for no expression
	move	$a0 $zero
	sw	$a0 12($s0)
				# Done Generating code for attribute x of type SELF_TYPE in class A
	move	$a0 $s0
				#Entered objectInitEpilogue
	lw	$fp 12($sp)
	lw	$s0 8($sp)
...
Main_init:
	...
	jr	$ra	
				...
				#Generating code for method init in class A
A.init:
	addiu	$sp $sp -4
	sw	$fp 0($sp)
	addiu	$sp $sp -4
	...
	addiu	$fp $sp 16
	move	$s0 $a0
				#Generating inner code for method init with AR_size of 12
	...
	sw	$a0 12($s0)
				...
				#Done Generating inner code for method init with AR_size of 12
	...
	addiu	$sp $sp 4
	jr	$ra	
				#Generating code for method foo in class A
A.foo:
	addiu	$sp $sp -4
	sw	$fp 0($sp)
	addiu	$sp $sp -4
	...
	addiu	$fp $sp 16
	move	$s0 $a0
				#Generating inner code for method foo with AR_size of 12
				#Entered cgen for int const expression
	la	$a0 int_const0
				#Leaving cgen for int const expression
				#Done Generating inner code for method foo with AR_size of 12
				#Incrementing Stack pointer and Restoring FP, SELF, and then jumping
	lw	$fp 12($sp)
	lw	$s0 8($sp)
	lw	$ra 4($sp)
	...
	addiu	$sp $sp 4
	jr	$ra	
				#Generating code for method getx in class A
A.getx:
	addiu	$sp $sp -4
	sw	$fp 0($sp)
	addiu	$sp $sp -4
	...
	addiu	$fp $sp 16
	move	$s0 $a0
				#Generating inner code for method getx with AR_size of 12
				#Entered cgen for object: x
	lw	$a0 12($s0)
				#Exited cgen for object
				#Done Generating inner code for method getx with AR_size of 12
				#Incrementing Stack pointer and Restoring FP, SELF, and then jumping
	lw	$fp 12($sp)
	lw	$s0 8($sp)
	lw	$ra 4($sp)
	...
	jr	$ra	
				...
				#Generating code for method foo in class B
B.foo:
	addiu	$sp $sp -4
	sw	$fp 0($sp)
	addiu	$sp $sp -4
