...
Bob_init:
	...
	move	$s0 $a0
				#Leaving objectInitPrologue
	jal	IO_init
				#Generating code for attribute x of type Int in class Bob
				#Entered and exited cgen for no expression
	move	$a0 $zero
	sw	$a0 12($s0)
				# Done Generating code for attribute x of type Int in class Bob
				#Generating code for attribute y of type Int in class Bob
				#Entered cgen for int const expression
	la	$a0 int_const0
				#Leaving cgen for int const expression
	sw	$a0 16($s0)
				# Done Generating code for attribute y of type Int in class Bob
	move	$a0 $s0
				#Entered objectInitEpilogue
	lw	$fp 12($sp)
	lw	$s0 8($sp)
...
Main_init:
	...
	move	$s0 $a0
				#Leaving objectInitPrologue
	jal	Bob_init
				#Generating code for attribute z of type Int in class Main
				#Entered cgen for int const expression
	la	$a0 int_const1
				#Leaving cgen for int const expression
	sw	$a0 20($s0)
				# Done Generating code for attribute z of type Int in class Main
	move	$a0 $s0
				#Entered objectInitEpilogue
	lw	$fp 12($sp)
	lw	$s0 8($sp)
	...
	jr	$ra	
				...
				#Entered codeClassMethods for Bob
				#Leaving codeClassMethods for Bob
				#Entered codeClassMethods for Main
				#Generating code for method jack in class Main
Main.jack:
	addiu	$sp $sp -12
	sw	$fp 0($sp)
	addiu	$sp $sp -4
	...
	addiu	$fp $sp 16
	move	$s0 $a0
				#Generating inner code for method jack with AR_size of 20
	...
	sw	$a0 16($s0)
				...
				#Done Generating inner code for method jack with AR_size of 20
	...
	addiu	$sp $sp 12
	jr	$ra	
				#Generating code for method print_z in class Main
Main.print_z:
	addiu	$sp $sp -8
	sw	$fp 0($sp)
	addiu	$sp $sp -4
	...
	addiu	$fp $sp 16
	move	$s0 $a0
				#Generating inner code for method print_z with AR_size of 16
				#Entered cgen for block
...
label2:
	...
	jalr	$t1
				#DONE dispatch for method out_string in class Main
				#Leaving cgen for block
				#Done Generating inner code for method print_z with AR_size of 16
				#Incrementing Stack pointer and Restoring FP, SELF, and then jumping
	lw	$fp 12($sp)
	lw	$s0 8($sp)
	lw	$ra 4($sp)
...
Main.main:
	...
	move	$s0 $a0
				...
				#Entered cgen for block
				#BEGIN dispatch for method print_z in class Main
				#Evaluating and pushing argument of type Int to current frame
				#Entered cgen for object: z
	lw	$a0 20($s0)
				#Exited cgen for object
...
label3:
	...
	lw	$t1 32($t1)
	jalr	$t1
				#DONE dispatch for method print_z in class Main
				#Leaving cgen for block
