...
Main_init:
	...
	jr	$ra	
				...
				#Generating code for method recite in class Main
Main.recite:
	addiu	$sp $sp -8
	sw	$fp 0($sp)
	addiu	$sp $sp -4
...
label1:
	...
	addiu	$sp $sp 8
	jr	$ra	
				#Generating code for method disregard in class Main
Main.disregard:
	addiu	$sp $sp -16
	sw	$fp 0($sp)
	addiu	$sp $sp -4
	...
	addiu	$fp $sp 16
	move	$s0 $a0
				#Generating inner code for method disregard with AR_size of 24
				#Entered cgen for object: self
	move	$a0 $s0
				#Exited cgen for object
				#Done Generating inner code for method disregard with AR_size of 24
				#Incrementing Stack pointer and Restoring FP, SELF, and then jumping
	lw	$fp 12($sp)
	lw	$s0 8($sp)
	lw	$ra 4($sp)
...
Main.main:
	...
	move	$s0 $a0
				...
				#BEGIN dispatch for method disregard in class Main
				#Evaluating and pushing argument of type Object to current frame
...
label2:
	...
	sw	$a0 0($sp)
	addiu	$sp $sp -4
				#Done pushing argument of type Object to current frame
				#Evaluating and pushing argument of type Object to current frame
				#BEGIN dispatch for method recite in class Main
				#Evaluating and pushing argument of type Int to current frame
				#Entered cgen for int const expression
...
label5:
	...
	lw	$t1 32($t1)
	jalr	$t1
				#DONE dispatch for method disregard in class Main
