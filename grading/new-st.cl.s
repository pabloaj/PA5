...
Main_init:
	...
	jr	$ra	
				...
				#Generating code for method printX in class A
A.printX:
	addiu	$sp $sp -4
	sw	$fp 0($sp)
	addiu	$sp $sp -4
	...
	addiu	$fp $sp 16
	move	$s0 $a0
				#Generating inner code for method printX with AR_size of 12
...
label2:
	...
	jalr	$t1
				...
				#Done Generating inner code for method printX with AR_size of 12
	...
	addiu	$sp $sp 4
	jr	$ra	
				#Generating code for method new_st in class A
A.new_st:
	addiu	$sp $sp -4
	sw	$fp 0($sp)
	addiu	$sp $sp -4
	...
	addiu	$fp $sp 16
	move	$s0 $a0
				#Generating inner code for method new_st with AR_size of 12
				#Entered cgen for new
	la	$a0 SELF_TYPE_protObj
	jal	Object.copy
	jal	SELF_TYPE_init
				#Leaving cgen for new
				#Done Generating inner code for method new_st with AR_size of 12
				#Incrementing Stack pointer and Restoring FP, SELF, and then jumping
	lw	$fp 12($sp)
	lw	$s0 8($sp)
	lw	$ra 4($sp)
	...
	addiu	$sp $sp 4
	jr	$ra	
				#Generating code for method bump in class A
A.bump:
	addiu	$sp $sp -4
	sw	$fp 0($sp)
	addiu	$sp $sp -4
	...
	addiu	$fp $sp 16
	move	$s0 $a0
				#Generating inner code for method bump with AR_size of 12
	...
	sw	$a0 12($s0)
				...
				#Done Generating inner code for method bump with AR_size of 12
				#Incrementing Stack pointer and Restoring FP, SELF, and then jumping
	lw	$fp 12($sp)
	lw	$s0 8($sp)
	lw	$ra 4($sp)
