...
B_protObj:
	...
	.word	-1
				#Entered codeProtObj for C
C_protObj:
	.word	7
	.word	3
	.word	C_dispTab
				#Leaving codeProtObj for C
	.word	-1
				#Entered codeProtObj for D
D_protObj:
	.word	8
	.word	3
	.word	D_dispTab
				#Leaving codeProtObj for D
	.globl	heap_start
heap_start:
	.word	0
	.text
...
C_init:
	...
	jr	$ra	
				#Leaving objectInitEpilogue
				#Leaving codeObjInit for C
				#Entered codeObjInit for D
D_init:
				#Entered objectInitPrologue
	addiu	$sp $sp -12
	sw	$fp 12($sp)
...
Main_init:
	...
	move	$s0 $a0
				#Leaving objectInitPrologue
	jal	IO_init
				#Generating code for attribute a of type A in class Main
	...
	sw	$a0 12($s0)
				# Done Generating code for attribute a of type A in class Main
	...
	sw	$a0 16($s0)
				...
				#Generating code for attribute c of type C in class Main
				#Entered cgen for new
	la	$a0 C_protObj
	jal	Object.copy
	jal	C_init
				#Leaving cgen for new
	sw	$a0 20($s0)
				# Done Generating code for attribute c of type C in class Main
				#Generating code for attribute d of type D in class Main
				#Entered cgen for new
	la	$a0 D_protObj
	jal	Object.copy
	jal	D_init
				#Leaving cgen for new
	sw	$a0 24($s0)
				# Done Generating code for attribute d of type D in class Main
	move	$a0 $s0
				#Entered objectInitEpilogue
	lw	$fp 12($sp)
	lw	$s0 8($sp)
...
A.f:
	...
	addiu	$fp $sp 16
	move	$s0 $a0
				#Generating inner code for method f with AR_size of 12
				#Entered cgen for int const expression
	la	$a0 int_const0
				#Leaving cgen for int const expression
				#Done Generating inner code for method f with AR_size of 12
...
A.g:
	...
	addiu	$fp $sp 16
	move	$s0 $a0
				#Generating inner code for method g with AR_size of 12
				#Entered cgen for int const expression
	la	$a0 int_const1
				#Leaving cgen for int const expression
				#Done Generating inner code for method g with AR_size of 12
				#Incrementing Stack pointer and Restoring FP, SELF, and then jumping
	lw	$fp 12($sp)
	lw	$s0 8($sp)
	lw	$ra 4($sp)
	...
	jr	$ra	
				...
				#Generating code for method g in class B
B.g:
	addiu	$sp $sp -4
	sw	$fp 0($sp)
	addiu	$sp $sp -4
	...
	jr	$ra	
				...
				#Entered codeClassMethods for C
				#Generating code for method f in class C
C.f:
	addiu	$sp $sp -4
	sw	$fp 0($sp)
	addiu	$sp $sp -4
	...
	addiu	$sp $sp 4
	jr	$ra	
				#Leaving codeClassMethods for C
				#Entered codeClassMethods for D
				#Generating code for method f in class D
D.f:
	addiu	$sp $sp -4
	sw	$fp 0($sp)
	addiu	$sp $sp -4
	...
	addiu	$sp $sp 4
	jr	$ra	
				#Generating code for method g in class D
D.g:
	addiu	$sp $sp -4
	sw	$fp 0($sp)
	addiu	$sp $sp -4
	...
	addiu	$sp $sp 4
	jr	$ra	
				#Leaving codeClassMethods for D
...
Main.main:
	...
	move	$s0 $a0
				...
				#BEGIN dispatch for method f in class A
...
label0:
	...
	lw	$t1 12($t1)
	jalr	$t1
				#DONE dispatch for method f in class A
	sw	$a0 0($sp)
	addiu	$sp $sp -4
...
label1:
	...
	jalr	$t1
				...
				#BEGIN dispatch for method out_int in class Main
				#Evaluating and pushing argument of type Int to current frame
				#BEGIN dispatch for method g in class A
				#Entered cgen for object: a
	lw	$a0 12($s0)
				#Exited cgen for object
	bne	$a0 $zero label2
...
label2:
	...
	lw	$t1 16($t1)
	jalr	$t1
				#DONE dispatch for method g in class A
	sw	$a0 0($sp)
	addiu	$sp $sp -4
				#Done pushing argument of type Int to current frame
				#Entered cgen for object: self
...
label7:
	...
	jalr	$t1
				...
				#BEGIN dispatch for method out_int in class Main
				#Evaluating and pushing argument of type Int to current frame
				#BEGIN dispatch for method f in class C
...
label8:
	...
	lw	$t1 12($t1)
	jalr	$t1
				#DONE dispatch for method f in class C
	sw	$a0 0($sp)
	addiu	$sp $sp -4
				#Done pushing argument of type Int to current frame
				#Entered cgen for object: self
...
label9:
	...
	jalr	$t1
				...
				#BEGIN dispatch for method out_int in class Main
				#Evaluating and pushing argument of type Int to current frame
				#BEGIN dispatch for method g in class C
				#Entered cgen for object: c
	lw	$a0 20($s0)
				#Exited cgen for object
	bne	$a0 $zero label10
...
label10:
	...
	lw	$t1 16($t1)
	jalr	$t1
				#DONE dispatch for method g in class C
	sw	$a0 0($sp)
	addiu	$sp $sp -4
				#Done pushing argument of type Int to current frame
				#Entered cgen for object: self
...
label11:
	...
	jalr	$t1
				...
				#BEGIN dispatch for method out_int in class Main
				#Evaluating and pushing argument of type Int to current frame
				#BEGIN dispatch for method f in class D
				#Entered cgen for object: d
	lw	$a0 24($s0)
				#Exited cgen for object
	bne	$a0 $zero label12
	la	$a0 str_const0
...
label12:
	...
	lw	$t1 12($t1)
	jalr	$t1
				#DONE dispatch for method f in class D
	sw	$a0 0($sp)
	addiu	$sp $sp -4
				#Done pushing argument of type Int to current frame
				#Entered cgen for object: self
...
label13:
	...
	jalr	$t1
				...
				#BEGIN dispatch for method out_int in class Main
				#Evaluating and pushing argument of type Int to current frame
				#BEGIN dispatch for method g in class D
				#Entered cgen for object: d
	lw	$a0 24($s0)
				#Exited cgen for object
	bne	$a0 $zero label14
...
label14:
	...
	lw	$t1 16($t1)
	jalr	$t1
				#DONE dispatch for method g in class D
	sw	$a0 0($sp)
	addiu	$sp $sp -4
				#Done pushing argument of type Int to current frame
				#Entered cgen for object: self
...
label23:
	...
	jalr	$t1
				...
				#BEGIN dispatch for method out_int in class Main
				#Evaluating and pushing argument of type Int to current frame
				#BEGIN static dispatch for method f in static class B
				#Entered cgen for object: c
	lw	$a0 20($s0)
				#Exited cgen for object
	bne	$a0 $zero label24
...
label24:
	...
	lw	$t1 12($t1)
	jalr	$t1
				#DONE dispatch for method f in static class B
	sw	$a0 0($sp)
	addiu	$sp $sp -4
				#Done pushing argument of type Int to current frame
				#Entered cgen for object: self
...
label25:
	...
	jalr	$t1
				...
				#BEGIN dispatch for method out_int in class Main
				#Evaluating and pushing argument of type Int to current frame
				#BEGIN static dispatch for method g in static class B
				#Entered cgen for object: c
	lw	$a0 20($s0)
				#Exited cgen for object
	bne	$a0 $zero label26
...
label26:
	...
	lw	$t1 16($t1)
	jalr	$t1
				#DONE dispatch for method g in static class B
	sw	$a0 0($sp)
	addiu	$sp $sp -4
				#Done pushing argument of type Int to current frame
				#Entered cgen for object: self
...
label27:
	...
	jalr	$t1
				...
				#BEGIN dispatch for method out_int in class Main
				#Evaluating and pushing argument of type Int to current frame
				#BEGIN static dispatch for method f in static class C
				#Entered cgen for object: d
	lw	$a0 24($s0)
				#Exited cgen for object
	bne	$a0 $zero label28
...
label28:
	...
	lw	$t1 12($t1)
	jalr	$t1
				#DONE dispatch for method f in static class C
	sw	$a0 0($sp)
	addiu	$sp $sp -4
				#Done pushing argument of type Int to current frame
				#Entered cgen for object: self
...
label29:
	...
	jalr	$t1
				...
				#BEGIN dispatch for method out_int in class Main
				#Evaluating and pushing argument of type Int to current frame
				#BEGIN static dispatch for method g in static class C
				#Entered cgen for object: d
	lw	$a0 24($s0)
				#Exited cgen for object
	bne	$a0 $zero label30
...
label30:
	...
	lw	$t1 16($t1)
	jalr	$t1
				#DONE dispatch for method g in static class C
	sw	$a0 0($sp)
	addiu	$sp $sp -4
				#Done pushing argument of type Int to current frame
				#Entered cgen for object: self
